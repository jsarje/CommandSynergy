...
public sealed class GenericPlaintextFormatProfileTests
{
    ...
    public void Generic_plaintext_profile_round_trips_core_structure()
    {
        ...
        var snapshot = new CommandSynergy.Application.Decks.Portability.PortableDeckSnapshot(
            ...
            Array.Empty<string>(),
            null,
            parsed.Entries.Select(entry => new CommandSynergy.Application.Decks.Portability.PortableDeckEntry(null, entry.OriginalLine, entry.DisplayName, entry.Quantity, entry.SectionId, entry.IsCommander, entry.IsCompanion, CommandSynergy.Application.Decks.Portability.ParseConfidence.Normalized)).ToArray(),

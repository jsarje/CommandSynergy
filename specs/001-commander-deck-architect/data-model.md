...
- **Purpose**: Server-owned Parquet-backed metadata store that holds authoritative local card records
  generated by curated imports and explicit Scryfall bulk-refresh workflows.
- **Fields**:
...
- **Validation Rules**:
  - Full snapshot replacement must be deterministic by card identifier.
  - Snapshot writes must complete atomically so readers never observe a partially regenerated file.
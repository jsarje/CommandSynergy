...
metadata to be refreshed through a separate Scryfall `oracle_cards` bulk-ingestion workflow while
runtime Scryfall fallback remains read-only.
...
**Performance Goals**: Search, validation, and analysis feedback within 2 seconds after deck mutations in at least 95% of test runs; search-index generation within current focused benchmark budget; reduce repeat Scryfall calls over time through periodic local bulk snapshot refreshes  
**Constraints**: Official Commander legality only for deck validation, bracket and Game Changers remain non-blocking guidance, commander selection must respect official commander eligibility exceptions, metadata failures must preserve user work, OWASP-aware external HTTP handling, no committed secrets, and local metadata must be refreshed via explicit bulk snapshot replacement rather than request-time writes  
...
- PASS: Performance budgets are explicit for search, validation, analysis, and the reduction of repeat Scryfall calls through explicit local bulk metadata refreshes.
...
- Treat Parquet as the authoritative local metadata store, refreshed by a separate bulk-ingestion workflow, with Scryfall as a read-only upstream fallback.
...
- Refresh the local Parquet snapshot explicitly from Scryfall's `oracle_cards` bulk feed and keep
  runtime Scryfall fallback read-only so normal usage does not mutate local storage.
...
- Implement Parquet snapshot replacement support in Infrastructure so the bulk-ingestion tool can
  regenerate the local snapshot deterministically.
...
- PASS: The design keeps .NET-first code quality expectations unchanged and uses existing project
  boundaries.
- PASS: Test impact is concrete: commander-selection eligibility, bulk snapshot regeneration,
  read-only fallback behavior, and degraded metadata paths all require regression coverage.
...
- PASS: Performance expectations remain explicit and are improved by reducing repeat Scryfall calls through explicit snapshot refreshes.
